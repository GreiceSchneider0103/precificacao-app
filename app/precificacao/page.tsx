...
function solvePOR(params: {
  ...
  const mc = porPago - comissaoVal - impostoVal - pisVal - channel.taxFixed - frete - cmv - operR$ - adsR$ + credFrete + credComissao + rebateVal;
  ...
  return {
    ...
    descontoNecessarioR$,
    breakdown: { comissao: comissaoVal, imposto: impostoVal, pisCofins: pisVal, taxaFixa: channel.taxFixed, frete, cmv, operacionais: operR$, ads: adsR$, creditoFrete: credFrete, creditoComissao: credComissao, rebate: rebateVal, margemContrib: mc, margemPct: mcPct, receitaLiquida },
  };
}
...
function mapRuleSetToSettings(rs: any): Settings {
  ...
  for (const k of keys) {
    ...
    channels[k] = {
      commissionPercent: Number(incoming.commissionPercent ?? 0),
      taxFixed: Number(incoming.taxFixed ?? 0),
      mainTaxPercent: Number(incoming.mainTaxPercent ?? mainTax),
      hasCredits: typeof incoming.hasCredits === "boolean" ? incoming.hasCredits : isMarketplace,
      creditFretePercent: Number(incoming.creditFretePercent ?? (isMarketplace ? 21.25 : 0)),
      creditCommissionPercent: Number(incoming.creditCommissionPercent ?? (isMarketplace ? 9.25 : 0)),
      targetMarginPercent: Number(incoming.targetMarginPercent ?? 10),
    };
    if (k === "meli") {
      channels[k].meli = {
  ...
  return { regime, ufOrigem: base.ufOrigem ?? "RS", channels } as Settings;
}

function InfoTip({ text }: { text: string }) {
  return (
    <span title={text} className="ml-1 inline-grid h-4 w-4 place-items-center rounded-full bg-white/5 text-[11px] text-white/70 ring-1 ring-white/10 cursor-help select-none" aria-label={text}>ℹ</span>
  );
}
...
export default function PrecificacaoPage() {
  ...
  // ✅ Campos CMV e nome manual (visíveis quando nenhum produto está selecionado)
  const [manualName, setManualName] = useState("");
  const [manualCmv, setManualCmv] = useState("");

  // ✅ Canal selecionado (agora com select visível no JSX)
  ...
  useEffect(() => {
    ...
    } catch (err) { console.error("Erro ao carregar draft", err); }
  }, []);
  ...
  useEffect(() => {
    if (selectedCouponId) {
      ...
      if (selected) {
        ...
        setDescontoValue(selected.discountMode === "percent" ? String(selected.discountValue) : String(selected.discountValue).replace(".", ","));
      }
    }
  ...
  useEffect(() => {
    const draft = { query, selectedSku, manualName, manualCmv, channel, meliMode, magaluShipMode, regimeOverride, frete, margem, operMode, operValue, adsMode, adsValue, descontoMode, descontoValue, selectedCouponId, rebateMode, rebateValue, commissionOverride, taxOverride, fixedOverride, creditFreteOverride, creditComissaoOverride };
  ...
  const result = useMemo(() => {
    ...
    const creditFretePercentBase = creditFreteOverride.trim() ? parseNumberPt(creditFreteOverride) : baseCh.creditFretePercent;
    const creditFretePercent = channel === "magalu" && magaluShipMode === "full" ? 0 : creditFretePercentBase;
    ...
    const ch = { commissionPercent, taxFixed, mainTaxPercent, hasCredits, creditFretePercent, creditCommissionPercent };
    ...
    if (shouldTierShopee) {
      const rTier = solveWithShopeeTiered({ cmv: effectiveCmv, markupBase, frete: freteN, operMode, operValue: operValueN, adsMode, adsValue: adsValueN, margemAlvoPercent: margemN, channel: ch, channelRaw: baseCh, regime: regimeFinal, rebateMode, rebateValue: rebateValueN, descontoMode, descontoValue: descontoValueN });
      return { ...rTier, regimeUsed: regimeFinal };
    }

    const r = solvePOR({ cmv: effectiveCmv, markupBase, frete: freteN, operMode, operValue: operValueN, adsMode, adsValue: adsValueN, margemAlvoPercent: margemN, channel: ch, regime: regimeFinal, rebateMode, rebateValue: rebateValueN, descontoMode, descontoValue: descontoValueN });
    return { ...r, channelUsed: ch, regimeUsed: regimeFinal };
  }, [settings, effectiveCmv, channel, meliMode, magaluShipMode, frete, operMode, operValue, adsMode, adsValue, margem, margemDirty, regimeOverride, descontoMode, descontoValue, rebateMode, rebateValue, commissionOverride, taxOverride, fixedOverride, creditFreteOverride, creditComissaoOverride, recalcTick]);
  ...
  const discountSimulation = useMemo(() => {
    ...
    const rebateVal = rebateMode === "percent" ? porPago * (parseNumberPt(rebateValue) / 100) : parseNumberPt(rebateValue);
    const mc = porPago - cVal - impostoVal - pisVal - ch.taxFixed - parseNumberPt(frete) - effectiveCmv - operR$ - adsR$ + credFrete + credComissao + rebateVal;
    const mcPct = porPago > 0 ? (mc / porPago) * 100 : 0;
    return { porPago, mcPct };
  }, [result, descontoMode, descontoValue, operMode, operValue, adsMode, adsValue, frete, effectiveCmv, rebateMode, rebateValue, recalcTick]);
  ...
  const alerts = useMemo(() => {
    ...
    if (commissionOverride.trim() || taxOverride.trim() || fixedOverride.trim()) list.push({ type: "warn", text: "Overrides ativos: você alterou taxas do padrão." });
    return list;
  }, [result, margem, effectiveCmv, commissionOverride, taxOverride, fixedOverride]);

  function saveToHistory(quiet = false) {
    if (!result) return false;
    const payload = { id: crypto.randomUUID(), createdAt: new Date().toISOString(), sku: picked?.sku || "", name: effectiveName, channel, meliMode, magaluShipMode, regime: result.regimeUsed, por: result.POR_sugerido, margemPct: result.breakdown.margemPct, cmv: effectiveCmv, frete: parseNumberPt(frete), operMode, operValue: parseNumberPt(operValue), adsMode, adsValue: parseNumberPt(adsValue), descontoMode, descontoValue: parseNumberPt(descontoValue), rebateMode, rebateValue: parseNumberPt(rebateValue), overrides: { commissionOverride: commissionOverride.trim(), taxOverride: taxOverride.trim(), fixedOverride: fixedOverride.trim(), creditFreteOverride: creditFreteOverride.trim(), creditComissaoOverride: creditComissaoOverride.trim() }, channelUsed: result.channelUsed };
    try {
  ...
  async function copyPorToClipboard() {
    if (!result) return;
    try {
      await navigator.clipboard.writeText(fmtPt(result.POR_sugerido));
  ...
  // Canais disponíveis (da settings ou fallback)
  const availableChannels = settings ? Object.keys(settings.channels) : ["magalu", "meli", "shopee"];
    ...
    <div className="space-y-5">
      ...
      <section className="relative overflow-visible rounded-2xl border border-white/10 bg-white/5 p-6">
        <div className="relative overflow-visible isolate grid gap-4 md:grid-cols-2">
          ...
          <div className="grid gap-3 content-start">
            ...
            {/* ✅ CANAL — select sempre visível */}
            <label className="grid gap-1">
              <span className="text-xs text-white/60">
                Canal de venda
                <InfoTip text="Selecione o marketplace ou canal para calcular as taxas corretas." />
              </span>
              <select
                value={channel}
                onChange={(e) => {
                  setChannel(e.target.value as ChannelKey);
                  setMargemDirty(false); // reseta margem ao trocar canal
                }}
            ...
            <div className="grid gap-1 relative">
              <label className="grid gap-1">
                <span className="text-xs text-white/60">
                  Buscar produto cadastrado (SKU ou nome)
                  <InfoTip text="Digite e selecione um produto cadastrado. Se não achar, preencha os campos manuais abaixo." />
                </span>
                <input
                  value={query}
                  onChange={(e) => {
            ...
            {/* ✅ CMV MANUAL + NOME — visíveis quando nenhum produto selecionado do banco */}
            {!picked && (
              <div className="rounded-xl border border-white/10 bg-white/5 p-4 grid gap-3">
                <p className="text-xs font-semibold text-white/60 uppercase tracking-wide">
                  Produto manual
                  <InfoTip text="Preencha quando o produto não está cadastrado na base. O CMV é obrigatório para o cálculo." />
                </p>
                <div className="grid gap-3 md:grid-cols-2">
                  <label className="grid gap-1">
                    ...
                    <input
                      ...
                      placeholder="ex: Poltrona Lisa NH"
                      className="rounded-xl bg-neutral-950/60 px-4 py-3 text-sm text-white ring-1 ring-white/10 outline-none focus:ring-2 focus:ring-blue-600/60"
                    />
                  </label>
                  <label className="grid gap-1">
                    <span className="text-xs text-white/60">
                      CMV (R$) <span className="text-rose-400">*</span>
                    </span>
                    <input
                      value={manualCmv}
                      ...
                      className="rounded-xl bg-neutral-950/60 px-4 py-3 text-sm text-white ring-1 ring-white/10 outline-none focus:ring-2 focus:ring-emerald-500/60"
                    />
                  </label>
                </div>
                {manualCmv && parseNumberPt(manualCmv) > 0 && (
                  <p className="text-xs text-emerald-400">
                    CMV: R$ {fmtPt(parseNumberPt(manualCmv))} — pronto para calcular.
                  </p>
                )}
              </div>
            )}
            ...
            {picked && (
              <div className="flex items-center justify-between gap-3 rounded-xl border border-emerald-500/20 bg-emerald-500/10 px-4 py-3">
                <div>
                  <p className="text-sm font-semibold text-white">{picked.name}</p>
                  <p className="text-xs text-white/60">SKU: {picked.sku} • CMV: R$ {fmtPt(picked.cmv)}</p>
                </div>
                <button
                  type="button"
              ...
              <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className={meliMode === "classic" ? "rounded-xl bg-white/15 px-3 py-2 text-xs font-semibold ring-1 ring-white/10" : "rounded-xl bg-white/5 px-3 py-2 text-xs font-semibold text-white/70 ring-1 ring-white/10 hover:bg-white/10"}
                  >
                    Clássico
                  </button>
                  <button
                    ...
                    className={meliMode === "premium" ? "rounded-xl bg-white/15 px-3 py-2 text-xs font-semibold ring-1 ring-white/10" : "rounded-xl bg-white/5 px-3 py-2 text-xs font-semibold text-white/70 ring-1 ring-white/10 hover:bg-white/10"}
                  >
                    Premium
                  </button>
                </div>
            ...
            {/* Magalu: envio próprio / Full */}
            {channel === "magalu" && (
              <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                <p className="text-xs text-white/60 font-semibold mb-2">Modalidade de envio — Magalu</p>
                <div className="flex gap-2">
                  <button type="button" onClick={() => setMagaluShipMode("proprio")} className={magaluShipMode === "proprio" ? "rounded-xl bg-white/15 px-3 py-2 text-xs font-semibold ring-1 ring-white/10" : "rounded-xl bg-white/5 px-3 py-2 text-xs font-semibold text-white/70 ring-1 ring-white/10 hover:bg-white/10"}>Envio próprio</button>
                  <button type="button" onClick={() => setMagaluShipMode("full")} className={magaluShipMode === "full" ? "rounded-xl bg-white/15 px-3 py-2 text-xs font-semibold ring-1 ring-white/10" : "rounded-xl bg-white/5 px-3 py-2 text-xs font-semibold text-white/70 ring-1 ring-white/10 hover:bg-white/10"}>Full Magalu</button>
                </div>
                <p className="mt-2 text-xs text-white/50">{magaluShipMode === "full" ? "No Full Magalu, o crédito de frete fica zerado." : "No envio próprio, o crédito de frete pode ser aplicado no Regime Normal."}</p>
              </div>
            )}
            ...
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <label className="grid gap-1">
                <span className="text-xs text-white/60">
                  Margem alvo (%)
                  <InfoTip text="Ao trocar canal, volta para o padrão do canal. Se digitar, fica travada." />
                </span>
                <input
                  value={margem}
                  onChange={(e) => { setMargem(e.target.value); setMargemDirty(true); }}
                  inputMode="decimal"
                  className="rounded-xl bg-neutral-950/60 px-4 py-3 text-sm text-white ring-1 ring-white/10 outline-none focus:ring-2 focus:ring-blue-600/60"
                />
              </label>
            ...
            {/* Custos, Ads, Desconto, Rebate */}
            <Accordion title="Custos, Ads, Desconto e Rebate" subtitle="Ajuste somente quando necessário." defaultOpen={false}>
              {/* Operacionais */}
              <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                <div className="flex items-center justify-between gap-3">
                  <p className="text-sm font-semibold">Custos operacionais<InfoTip text="Custo operacional por item." /></p>
                  <div className="flex gap-2">
                    <button type="button" onClick={() => setOperMode("fixed")} className={operMode === "fixed" ? "rounded-xl bg-white/15 px-3 py-2 text-xs font-semibold ring-1 ring-white/10" : "rounded-xl bg-white/5 px-3 py-2 text-xs font-semibold text-white/70 ring-1 ring-white/10 hover:bg-white/10"}>R$</button>
                    <button type="button" onClick={() => setOperMode("percent")} className={operMode === "percent" ? "rounded-xl bg-white/15 px-3 py-2 text-xs font-semibold ring-1 ring-white/10" : "rounded-xl bg-white/5 px-3 py-2 text-xs font-semibold text-white/70 ring-1 ring-white/10 hover:bg-white/10"}>%</button>
                  </div>
                </div>
                <input value={operValue} onChange={(e) => setOperValue(e.target.value)} inputMode="decimal" placeholder={operMode === "percent" ? "ex: 2,5" : "ex: 12,00"} className="mt-3 w-full rounded-xl bg-neutral-950/60 px-4 py-3 text-sm text-white ring-1 ring-white/10 outline-none focus:ring-2 focus:ring-blue-600/60" />
              </div>

              {/* Ads */}
              <div className="mt-3 rounded-xl border border-white/10 bg-white/5 p-4">
                <div className="flex items-center justify-between gap-3">
                  <p className="text-sm font-semibold">Ads<InfoTip text="Custo de mídia por item." /></p>
                  <div className="flex gap-2">
                    <button type="button" onClick={() => setAdsMode("fixed")} className={adsMode === "fixed" ? "rounded-xl bg-white/15 px-3 py-2 text-xs font-semibold ring-1 ring-white/10" : "rounded-xl bg-white/5 px-3 py-2 text-xs font-semibold text-white/70 ring-1 ring-white/10 hover:bg-white/10"}>R$</button>
                    <button type="button" onClick={() => setAdsMode("percent")} className={adsMode === "percent" ? "rounded-xl bg-white/15 px-3 py-2 text-xs font-semibold ring-1 ring-white/10" : "rounded-xl bg-white/5 px-3 py-2 text-xs font-semibold text-white/70 ring-1 ring-white/10 hover:bg-white/10"}>%</button>
                  </div>
                </div>
                <input value={adsValue} onChange={(e) => setAdsValue(e.target.value)} inputMode="decimal" placeholder={adsMode === "percent" ? "ex: 3" : "ex: 25,00"} className="mt-3 w-full rounded-xl bg-neutral-950/60 px-4 py-3 text-sm text-white ring-1 ring-white/10 outline-none focus:ring-2 focus:ring-blue-600/60" />
              </div>

              {/* Desconto + Rebate */}
              <div className="mt-3 grid gap-3 md:grid-cols-2">
                <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                  <p className="text-sm font-semibold">Desconto/Cupom<InfoTip text="Simula a margem com desconto no DE." /></p>
                  {coupons.length > 0 && (
                    <div className="mt-3 grid gap-2">
                      <label className="text-xs text-white/60">Cupons disponíveis</label>
                      <select value={selectedCouponId || ""} onChange={(e) => setSelectedCouponId(e.target.value || null)} className="rounded-xl bg-neutral-950/60 px-3 py-2 text-sm text-white ring-1 ring-white/10 outline-none">
                        <option value="">— Sem cupom —</option>
                        {coupons.map((c) => (
                          <option key={c.id} value={c.id}>{c.name} ({c.code}) - {c.discountMode === "percent" ? `${c.discountValue}%` : `R$ ${fmtPt(c.discountValue)}`}</option>
                        ))}
                      </select>
                    </div>
                  )}
                  ...
                  <div className="mt-2 flex gap-2">
                    <button type="button" onClick={() => setDescontoMode("percent")} className={descontoMode === "percent" ? "rounded-xl bg-white/15 px-3 py-2 text-xs font-semibold ring-1 ring-white/10" : "rounded-xl bg-white/5 px-3 py-2 text-xs font-semibold text-white/70 ring-1 ring-white/10 hover:bg-white/10"}>%</button>
                    <button type="button" onClick={() => setDescontoMode("fixed")} className={descontoMode === "fixed" ? "rounded-xl bg-white/15 px-3 py-2 text-xs font-semibold ring-1 ring-white/10" : "rounded-xl bg-white/5 px-3 py-2 text-xs font-semibold text-white/70 ring-1 ring-white/10 hover:bg-white/10"}>R$</button>
                  </div>
                  <input value={descontoValue} onChange={(e) => { setDescontoValue(e.target.value); setSelectedCouponId(null); }} inputMode="decimal" placeholder={descontoMode === "percent" ? "ex: 10" : "ex: 25,00"} className="mt-3 w-full rounded-xl bg-neutral-950/60 px-4 py-3 text-sm text-white ring-1 ring-white/10 outline-none" />
                </div>
                <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                  <p className="text-sm font-semibold">Rebate<InfoTip text="Crédito adicional somado na margem." /></p>
                  <div className="mt-3 flex gap-2">
                    <button type="button" onClick={() => setRebateMode("percent")} className={rebateMode === "percent" ? "rounded-xl bg-white/15 px-3 py-2 text-xs font-semibold ring-1 ring-white/10" : "rounded-xl bg-white/5 px-3 py-2 text-xs font-semibold text-white/70 ring-1 ring-white/10 hover:bg-white/10"}>%</button>
                    <button type="button" onClick={() => setRebateMode("fixed")} className={rebateMode === "fixed" ? "rounded-xl bg-white/15 px-3 py-2 text-xs font-semibold ring-1 ring-white/10" : "rounded-xl bg-white/5 px-3 py-2 text-xs font-semibold text-white/70 ring-1 ring-white/10 hover:bg-white/10"}>R$</button>
                  </div>
                  <input value={rebateValue} onChange={(e) => setRebateValue(e.target.value)} inputMode="decimal" placeholder={rebateMode === "percent" ? "ex: 4,5" : "ex: 20,00"} className="mt-3 w-full rounded-xl bg-neutral-950/60 px-4 py-3 text-sm text-white ring-1 ring-white/10 outline-none" />
                </div>
              </div>
            </Accordion>
          ...
          <div className="relative z-0 rounded-2xl border border-white/10 bg-white/5 p-5">
            ...
            {!result ? (
              <div className="mt-4 rounded-xl border border-white/10 bg-white/5 p-4 text-sm text-white/70">
                {!settings
                  ? "Configure as Configurações e salve antes de calcular."
                  : !effectiveCmv || effectiveCmv <= 0
                  ? "Informe o CMV do produto (manual ou selecione um produto cadastrado)."
                  : "Informe um CMV válido e verifique se Configurações estão salvas."}
              </div>
            ) : (
              <>
                {alerts.length > 0 && (
                  <div className="mt-4 space-y-2">
                    {alerts.map((a, i) => (
                      <div key={i} className={a.type === "bad" ? "rounded-xl border border-rose-500/25 bg-rose-500/10 px-4 py-2 text-sm text-rose-100" : "rounded-xl border border-amber-500/25 bg-amber-500/10 px-4 py-2 text-sm text-amber-100"}>{a.text}</div>
                    ))}
                  </div>
                )}
                ...
                <div className="mt-4 rounded-2xl border border-white/10 bg-white/5 p-5">
                  ...
                  <div className="mt-4 flex items-center justify-between gap-3">
                    <div>
                      <p className="text-xs text-white/60">Preço POR (sugerido)<InfoTip text="POR = preço final ao cliente. Esse é o número que você publica." /></p>
                    ...
                    <button type="button" onClick={copyPorToClipboard} className="h-11 shrink-0 rounded-xl bg-white/5 px-4 text-sm font-semibold text-white/85 ring-1 ring-white/10 hover:bg-white/10">Copiar POR</button>
                  </div>

                  <button onClick={() => saveToHistory(false)} className="mt-4 w-full rounded-xl bg-emerald-500/15 px-4 py-3 text-sm font-semibold text-emerald-200 ring-1 ring-emerald-500/20 hover:bg-emerald-500/20 transition">Salvar no histórico</button>
                  <button type="button" onClick={() => { const ok = saveToHistory(true); if (ok) { resetForNewPricing(); showToast("ok", "Salvo!"); } }} className="mt-2 w-full rounded-xl bg-blue-500/12 px-4 py-3 text-sm font-semibold text-blue-100 ring-1 ring-blue-500/20 hover:bg-blue-500/16 transition">Salvar e nova precificação</button>
                </div>
                ...
                <div className="mt-4 rounded-2xl border border-blue-500/20 bg-blue-500/10 p-5">
                  ...
                  <div className="mt-3 grid gap-3 md:grid-cols-2">
                    ...
                    <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                      ...
                      <p className="mt-1 text-xl font-semibold">{result.descontoNecessarioPct.toFixed(2)}% <span className="text-sm text-white/60">(R$ {fmtPt(result.descontoNecessarioR$)})</span></p>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-4 space-y-2 text-sm">
                  ...
                  <Row label={`PIS/COFINS (9,25%) ${result.regimeUsed === "normal" ? "" : "(não aplica)"}`} value={`R$ ${fmtPt(result.breakdown.pisCofins)}`} />
      ...
      {toast && (
        <div className="fixed bottom-4 right-4 z-50">
          <div className={"rounded-xl px-4 py-3 text-sm font-semibold shadow-lg ring-1 " + (toast.type === "ok" ? "bg-emerald-500/15 text-emerald-100 ring-emerald-500/30" : "bg-rose-500/15 text-rose-100 ring-rose-500/30")}>
            {toast.text}
          </div>
        </div>
      )}
